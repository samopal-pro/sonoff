...
void Sha256Class::add(char *s) {
  for( int i=0; s[i]!='\0'; i++)write1(s[i]);
 }



void Sha256Class::write1(uint8_t data) {
  ++byteCount;
  addUncounted(data);
}
...
uint8_t* Sha256Class::resultHmac(void) {
  ...
  init();
  for (i=0; i<BLOCK_LENGTH; i++) write1(keyBuffer[i] ^ HMAC_OPAD);
  for (i=0; i<HASH_LENGTH; i++) write1(innerHash[i]);
  return result();
}

char * Sha256Class::char_result() {
  uint8_t* hash = result();
  for ( int i=0; i<HASH_LENGTH; i++) {
    charBuffer[i*2] = "0123456789abcdef"[hash[i]>>4];
    charBuffer[i*2+1] = "0123456789abcdef"[hash[i]&0xf];
  }
  charBuffer[BLOCK_LENGTH] = '\0';
  return charBuffer;
} 

char * Sha256Class::char_resultHmac() {
  uint8_t* hash = resultHmac();
  for ( int i=0; i<HASH_LENGTH; i++) {
    charBuffer[i*2] = "0123456789abcdef"[hash[i]>>4];
    charBuffer[i*2+1] = "0123456789abcdef"[hash[i]&0xf];
  }
